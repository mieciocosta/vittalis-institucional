...
function Anim({ children, d = 0, className = "" }: { children: React.ReactNode; d?: number; className?: string }) {
  ...
  return <div ref={ref} className={className} style={{ opacity: 1, transform: "translateY(0)", transition: `all .65s ease ${d}s` }}>{children}</div>;
}
...
export default function SpecialtyPageClient({ specialty: s }: { specialty: Specialty }) {
  ...
  return (
    <div style={{ fontFamily: "var(--font-body)", color: "var(--vit-charcoal)" }}>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 66 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <a href={waUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, fontWeight: 600, background: "var(--vit-primary)", color: "#fff", padding: "9px 22px", borderRadius: 100, textDecoration: "none", transition: "all .3s" }}
              onMouseEnter={e=>e.currentTarget.style.background="var(--vit-primary-dark)"} onMouseLeave={e=>e.currentTarget.style.background="var(--vit-primary)"} onClick={() => trackEvent("click_whatsapp", { source: "navbar", specialty: s.slug })}>
              {s.ctaText}
            </a>
          </div>
        </div>
      ...
      <div style={{ position: "fixed", bottom: 20, right: 20, zIndex: 1000, display: "flex", flexDirection: "column", gap: 12, alignItems: "flex-end" }}>
        <a href="https://www.instagram.com/vittalissaudeslz/" target="_blank" rel="noopener noreferrer" aria-label="Instagram da Vittalis" onClick={() => trackEvent("click_instagram", { source: "fab", specialty: s.slug })}
          style={{ width: 48, height: 48, borderRadius: "50%", background: "linear-gradient(135deg, #833AB4, #E1306C, #F77737)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(225,48,108,.3)", transition: "transform .3s", textDecoration: "none" }}
        ...
        <a href={waUrl} target="_blank" rel="noopener noreferrer" aria-label="Agendar pelo WhatsApp" onClick={() => trackEvent("click_whatsapp", { source: "fab", specialty: s.slug })}
          style={{ width: 56, height: 56, borderRadius: "50%", background: "#25D366", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 18px rgba(37,211,102,.4)", transition: "transform .3s", textDecoration: "none" }}
          onMouseEnter={e=>e.currentTarget.style.transform="scale(1.1)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>
